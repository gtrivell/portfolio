A movie recommender using Python.

Scrapes through users' data and makes recommendations based on the genre, director, actors, etc
of movies they rated highly. Could also make recommendations based off the viewing habits of 
users with similar taste.

Details in report.
____________________________________________________________________

test.dat is the same test file given on miner, just renamed
for convenience when using in HW4.py. 
